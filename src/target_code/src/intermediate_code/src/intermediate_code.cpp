...
namespace target_code
{
    ...
    void LogWarning(std::string msg, unsigned int line) {
        PRECONDITION(line >= 0);
        std::cout << "\033[33mWarning, in line: " << line << ":\033[0m "
            << msg << std::endl;
    }
    ...
    expression::Symbol* NewTemp(expression::ExprType type,
       ...
       std::string id = NewTempName();
        ...
        if (new_temp == nullptr)  
            new_temp = syntax_analysis::DefineNewSymbol(type, id.c_str(), index,
                TEMP_LINE);
    ...
    Quad* Emit(Iopcode op, expression::Expression* result,
        expression::Expression* arg1, expression::Expression* arg2,
            unsigned int line) {
        PRECONDITION(line >= 0);
    ...
    expression::Symbol* MemberItem(expression::Symbol* sym, const char* id,
        unsigned int line) {
        PRECONDITION(sym != nullptr);
    ...
    std::list<expression::Expression*> params, unsigned int line) {
        ...
        if (IsUserFunction(called_symbol)) {
            ...
            auto func_def_args_num = 
            called_symbol->get_formal_arguments().size();
            if (call_args_num < func_def_args_num) 
                syntax_analysis::SignalError(
                    "Too few arguments passed to function: " 
                    + called_symbol->get_id() 
                    + ", defined in line: " 
                    + std::to_string(called_symbol->get_line()), line);
            else if (call_args_num > func_def_args_num) 
                LogWarning("Too many arguments passed to function: " 
                + called_symbol->get_id() + ", defined in line: " 
                + std::to_string(called_symbol->get_line()), line);
        }
    }
    ...
    expression::Symbol* ConcludeShortCircuit(expression::BoolExpr* expr,
        ...
        Emit(ASSIGN_t, temp, new expression::BoolConstant(false),
            nullptr, line);

        return temp;
    }

    bool IsValidArithmetic(expression::Expression* expr, std::string context,
        unsigned int line) {
        assert (expr != nullptr);
        if (IsLibraryFunction(expr)) {
            syntax_analysis::SignalError("Invalid use of " + context 
            + " operator on library function " + expr->to_string(), line);
            return false;
        }
        else if (IsUserFunction(expr)) {
            syntax_analysis::SignalError("Invalid use of " + context 
            + " operator on user function " + expr->to_string(), line);
            return false;
        }
        else if (IsConstString(expr)) {
            syntax_analysis::SignalError("Invalid use of " + context 
            + " operator on const string " + expr->to_string(), line);
            return false;
        }
        else if (IsConstBool(expr)) {
            syntax_analysis::SignalError("Invalid use of " + context 
            + " operator on const bool " + expr->to_string(), line);
            return false;
        }
        else if (IsTableMake(expr)) {
            syntax_analysis::SignalError("Invalid use of " + context 
            + " operator on table " + expr->to_string(), line);
            return false;
        }
    ...
    bool IsValidArithmeticComp(expression::Expression* expr, 
        unsigned int line) {
        PRECONDITION(expr != nullptr);
    ...
    bool IsValidAssign(expression::Symbol* left_operand, unsigned int line) {
        ...
        if (IsUserFunction(left_operand) || IsLibraryFunction(left_operand)) {
            syntax_analysis::SignalError(
                "Functions are constant their value cannot be changed", line);
    ...
    bool IsValidBreakContinue() {
        return (stmt_stack.empty() 
            || stmt_stack.back() != LOOP_t) ? false : true;
    }
